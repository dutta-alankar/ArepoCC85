...
/*! \brief Main routine to compute fluxes across interfaces given a tessellated mesh T.
 *
...
void compute_interface_fluxes(tessellation *T)
{
  ...
  face *VF  = T->VF; /* voronoi face  struct array in the tessellated mesh */
  point *DP = T->DP; /* delauny point struct array in the tessellated mesh */

  for(i = 0; i < T->Nvf; i++) /* loop over all the faces */
    {
      ...
      double mass_fluxes, mom_fluxes, enrg_fluxes, p_wind;
      ...
      mom_fluxes   = Pdot / (4 * M_PI * opn * radius2);;
      enrg_fluxes  = Edot / (4 * M_PI * opn * radius2);
      p_wind  = 2*sqrt(Mdot*Edot)*pow(Rinj,-2)*4.50e-2;
...
#ifdef AGNWIND_FLAG
      if((state_L.flga != 1) && (state_R.flga != 1))
	   face_limit_fluxes(&state_L, &state_R, &state_center_L, &state_center_R, &fluxes, face_dt, &count, &count_reduced);
      if(state_L.flga == 1 && state_R.flga == 1){
        fluxes.mass = 0.;
        fluxes.momentum[0] = 0.;
      ...
      if(state_L.flga == 1 && state_R.flga == 2){
        ...
        fluxes.momentum[2] =  mom_fluxes * geom.nz + p_wind * geom.nz;
        fluxes.energy      =  enrg_fluxes;
      ...
      if(state_L.flga == 2 && state_R.flga == 1){
        ...
        fluxes.momentum[2] =  mass_fluxes * geom.nz + p_wind * geom.nz;
        fluxes.energy      = -enrg_fluxes;
...
#ifdef AGNWIND_FLAG
      st->flga = PrimExch[particle].AGNFlag;
#endif /* ifdef AGNWIND_FLAG */
    }
...
void face_set_scalar_states_and_fluxes(struct state *st_L, struct state *st_R, struct state_face *st_face, struct fluxes *flux)
{
  ...
  if(flux->mass > 0) /* upwind advection */

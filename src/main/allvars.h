...
/* This is where all the global variables are declared so that they are known to all other pieces of code.
 * However assigning values to them later on requires them to be defined first as memory allocation for the
 * variables aren't done here.
 */
...
#if defined(VORONOI_MESHOUTPUT)
#ifndef VORONOI_MESH_KEEP_DT_AND_DTC
#error \
    "VORONOI_MESHOUTPUT requires VORONOI_MESH_KEEP_DT_AND_DTC."
#endif /* #ifndef VORONOI_MESH_KEEP_DT_AND_DTC */
#endif /* #if defined(VORONOI_MESHOUTPUT) */
...
#define terminate(...)                                                                                                            \
  {                                                                                                                               \
    if(FlagNyt == 0)                                                                                                              \
      {                                                                                                                           \
        char termbuf1[4*MAXLEN_PATH+10000], termbuf2[5*MAXLEN_PATH+10000];                                                                                      \
...
#define warn(...)                                                                                                            \
  {                                                                                                                          \
    char termbuf1[3*MAXLEN_PATH+1000], termbuf2[3*MAXLEN_PATH+1000];                                                                                     \
...
/* periodicity of gas */
#ifndef REFLECTIVE_X
#define NGB_PERIODIC_LONG_X(x) ( (fabs(x) > boxHalf_X) ? (boxSize_X - fabs(x)) : fabs(x))
#define NEAREST_X(x) ( ((x) > boxHalf_X) ? ((x) - boxSize_X) : (((x) < -boxHalf_X) ? ((x) + boxSize_X) : (x)))
#define WRAP_X(x) ( ((x) > boxSize_X) ? ((x) - boxSize_X) : (((x) < 0) ? ((x) + boxSize_X) : (x)))
...
#ifndef REFLECTIVE_Y
#define NGB_PERIODIC_LONG_Y(y) ((fabs(y) > boxHalf_Y) ? (boxSize_Y - fabs(y)) : fabs(y))
#define NEAREST_Y(y) (((y) > boxHalf_Y) ? ((y) - boxSize_Y) : (((y) < -boxHalf_Y) ? ((y) + boxSize_Y) : (y)))
#define WRAP_Y(y) ( ((y) > boxSize_Y) ? ((y) - boxSize_Y) : (((y) < 0) ? ((y) + boxSize_Y) : (y)))
...
#ifndef REFLECTIVE_Z
#define NGB_PERIODIC_LONG_Z(z) ( (fabs(z) > boxHalf_Z) ? (boxSize_Z - fabs(z)) : fabs(z))
#define NEAREST_Z(z) (((z) > boxHalf_Z) ? ((z) - boxSize_Z) : (((z) < -boxHalf_Z) ? ((z) + boxSize_Z) : (z)))
#define WRAP_Z(z) (  ((z) > boxSize_Z) ? ((z) - boxSize_Z) : (((z) < 0) ? ((z) + boxSize_Z) : (z)))
...
#ifdef AGNWIND_FLAG
extern double SpherePosX,SpherePosY,SpherePosZ;

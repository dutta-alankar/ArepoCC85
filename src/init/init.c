...
#ifdef STICKYFLAGS
  if(RestartFlag == 0 || RestartFlag == 1 || RestartFlag == 2)
    {
      for(i = 0; i < NumPart; i++)
        {
          /* check whether particle is within stickly layer MaxDist
 *            * if yes, set its StickyFlag */
  ...
  if(RestartFlag == 18) /* recalculation of potential */
    {
      ...
      strcat(All.SnapshotFileBase, "_potupdated");
  ...
  if(RestartFlag == 14)
    {
      char tess_name[MAXLEN_PATH+1024];
  ...
  for(i = 0, mass = 0; i < NumGas; i++)
    {
      ...
      for(j = 0; j < COUNT_PASSIVE_SCALARS; j++)

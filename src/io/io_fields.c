...
static void io_func_coolrate(int particle, int components, void *buffer, int mode)
{
  ...
  /* get cooling time */
  tcool = GetCoolingTime(SphP[particle].Utherm, SphP[particle].Density * All.cf_a3inv);
...
#ifdef AGNWIND_FLAG
  init_field(IO_AGNFLAGA, "FLGA", "AGNFlag", MEM_INT, FILE_INT, FILE_INT, 1, A_SPHP, &SphP[0].AGNFlag, 0, GAS_ONLY);
...
#endif /* #ifdef OUTPUT_STICKYFLAGS */
}

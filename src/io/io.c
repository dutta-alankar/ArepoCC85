                ...
                void *pointer_to_field, void (*io_func)(int, int, void *, int), int typelist_bitmask)
{
  ...
  IO_Fields[N_IO_Fields].label[4] = '\0';
  strncpy(IO_Fields[N_IO_Fields].datasetname, datasetname, 256);
  IO_Fields[N_IO_Fields].datasetname[256] = '\0';
  ...
  for(f = 0; f < N_IO_Fields; f++)
    {
      if(IO_Fields[f].field == blocknr)
        {
          strncpy(label, IO_Fields[f].label, 5);
          return;
        }
    }
...
{
  FILE *f;
  char buf[4*MAXLEN_PATH+1000], buf2[3*MAXLEN_PATH+256];
  int i;
  int npresent[NTYPES];
 ...
 *  \return void
 */
void file_path_sprintf(char *buf, const char *fmt, ...)
{
  va_list l;
  va_start(l, fmt);

...
#define INT 3
  ...
  FILE *fd, *fdout;
  char buf[MAXLEN_PARAM_TAG + MAXLEN_PARAM_VALUE + MAXLEN_PATH + 200], buf1[MAXLEN_PARAM_TAG + MAXLEN_PATH + 200], buf2[MAXLEN_PARAM_VALUE + MAXLEN_PATH + 200],
      buf3[MAXLEN_PARAM_TAG + MAXLEN_PARAM_VALUE + 3*MAXLEN_PATH + 1000];
  int i, j, nt;
  ...
  if(ThisTask == 0) /* read parameter file on process 0 */
    {
...
#ifdef AGNWIND_FLAG
      strcpy(tag[nt],"AGNWindMdot");
      addr[nt] = &All.AGNWindMdot;
      id[nt++] = REAL;

      strcpy(tag[nt],"AGNWindEdot");
      addr[nt] = &All.AGNWindEdot;
      id[nt++] = REAL;

      strcpy(tag[nt],"AGNWindPdot");
      addr[nt] = &All.AGNWindPdot;
      id[nt++] = REAL;

      strcpy(tag[nt],"AGNWindSphereRad");
      addr[nt] = &All.AGNWindSphereRad;
      id[nt++] = REAL;
      ...
      addr[nt] = &All.TimeBeginAGN;
      id[nt++] = REAL;

      strcpy(tag[nt],"AGNDutyCycle");
      addr[nt] = &All.AGNDutyCycle;
      id[nt++] = REAL;

      strcpy(tag[nt],"AGNLifetime");
      addr[nt] = &All.AGNLifetime;
      id[nt++] = REAL;
      ...
      if((fd = fopen(fname, "r")))
        {
          ...
          if(!(fdout = fopen(buf, "w")))
          ...
          else
            {
              ...
              while(!feof(fd))
                {
                  *buf = 0;
				  ...
				  if(!status && ferror(fd))
				   mpi_terminate("Error obtaining parameters from file '%s' %s:\n", fname, buf);
              ...
              if(errorFlag == 0)
                if( system(buf3) != 0)
				 mpi_terminate("Error occured in running system commands.\n");

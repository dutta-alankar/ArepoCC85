...
void find_timesteps_without_gravity(void)
{
  ...
  for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
    ...
    if(All.NumCurrentTiStep > 0){
      ...
      minLocal = All.HighestOccupiedTimeBin;
      for(idx = 0; idx < TimeBinsHydro.NActiveParticles; idx++)
      {
        i = TimeBinsHydro.ActiveParticleList[idx];
        if(i < 0)
	  ...
	  if(SphP[i].AGNFlag != 1)
	   if(P[i].TimeBinHydro < minLocal)
	    minLocal = P[i].TimeBinHydro;
      }
...
#elif !((defined(SELFGRAVITY) || defined(EXTERNALGRAVITY) || defined(EXACT_GRAVITY_FOR_PARTICLE_TYPE))) || defined(MESHRELAX) /* #ifdef FORCE_EQUAL_TIMESTEPS */
  return;
